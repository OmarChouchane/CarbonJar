...
// ENUM DEFINITIONS
// =====================
export const authUserRole = pgEnum('auth_user_role', ['admin', 'trainer', 'trainee', 'user']);
...
export const courseLevel = pgEnum('course_level', ['beginner', 'intermediate', 'expert']);
...
export const assessmentType = pgEnum('assessment_type', ['Quiz', 'Exam', 'Assignment']);
export const deliveryMode = pgEnum('delivery_mode', ['self_paced', 'live', 'hybrid']);
...
export const courses = pgTable(
  'courses',
  {
    ...
    carbonTopicId: uuid('carbon_topic_id').references(() => carbonTopics.topicId),
  ...
  (table) => ({
    ...
    carbonTopicIdx: index('idx_courses_carbon_topic_id').on(table.carbonTopicId),
  }),
);
...
export const questions = pgTable(
  ...
  (table) => ({
    assessmentIdIdx: index('idx_questions_assessment_id').on(table.assessmentId),
  }),
);
...
export const certificates = pgTable(
  'certificates',
  {
    ...
    issuerName: varchar('issuer_name', { length: 255 }).default('Ussama Ben Abdessalem'),
    issuerRole: varchar('issuer_role', { length: 255 }).default('CEO of Carbon Jar'),

    certificateCode: varchar('certificate_code', { length: 32 }).unique().notNull(), // e.g., "0164"
    certificateSlug: varchar('certificate_slug', { length: 255 }).unique().notNull(), // e.g., "molk-souli-intro-to-lca"
...
export const trainingSessions = pgTable(
  ...
  (table) => ({
    ...
    instructorIdIdx: index('idx_training_sessions_instructor_id').on(table.instructorId),
  }),
);
